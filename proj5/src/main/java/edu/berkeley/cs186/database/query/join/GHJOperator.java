...
public class GHJOperator extends JoinOperator {
    ...
    public int estimateIOCost() {
        ...
        // E.g., when join the gender column.     ---By hjt
    ...
    private void partition(Partition[] partitions, Iterable<Record> records, boolean left, int pass) {
        ...
        for (Record record : records) {
            int columnValueIndex = left ? getLeftColumnIndex() : getRightColumnIndex();
            DataBox columnValue = record.getValue(columnValueIndex);
    ...
    private void buildAndProbe(Partition leftPartition, Partition rightPartition) {
        ...
        for (Record buildRecord : buildRecords) {
            ...
            if (!hashTable.containsKey(buildValue)) {
                hashTable.put(buildValue, new ArrayList<>());
            }
            hashTable.get(buildValue).add(buildRecord);
        }

        // Probing stage
        for (Record probeRecord : probeRecords) {
            DataBox probeValue = probeRecord.getValue(probeColumnIndex);
            if (!hashTable.containsKey(probeValue)) continue;
            for (Record buildRecord : hashTable.get(probeValue)) {
                Record joinRecord;
                if (probeFirst) {
    ...
    public static Pair<List<Record>, List<Record>> getBreakSHJInputs() {
        ...
        // In SHJ, first hash stage need B-1 buffers, and the rehash stage need B-2 buffers
        // (B - 1) * (B - 2) * 8 + 1 = 161.
        for (int i = 0; i < 161; i++) {
    ...
    public static Pair<List<Record>, List<Record>> getBreakGHJInputs() {
        ...
        // When the join column values are all the same, you will get the same partition
        // no matter how many times you hash it. So as long as the number of partition pages
        // obtained in the first hash exceeds the number of buffer GHS will fail.    ----By hjt
        // (B - 2) * 8 + 1 = 33.
        for (int i = 0; i < 33; i++) {
