...
class InnerNode extends BPlusNode {
    ...
    // Core API ////////////////////////////////////////////////////////////////
    // See BPlusNode.get.
    // Project failed at the first time       By hjt
    @Override
    ...
    public LeafNode getLeftmostLeaf() {
        ...
        BPlusNode leftmostChild = BPlusNode.fromBytes(metadata, bufferManager, treeContext, pageNum);
    ...
    private Optional<Pair<DataBox, Long>> insert(DataBox key, Long child) {
        ...
        if (keys.size() <= metadata.getOrder() * 2) {
            // Hit the case 1 that described in BPLusNode::put.
            sync();
            return Optional.empty();
        } else {
            // Hit the case 2 that described in BPLusNode::put.
            DataBox newKey = keys.get(metadata.getOrder());

            List<DataBox> newSiblingNodeKeys =
                    keys.subList(metadata.getOrder() + 1, keys.size());
            List<Long> newSiblingNodeChildren =
            ...
            InnerNode newSiblingNode = new InnerNode(metadata, bufferManager,
                    newSiblingNodeKeys, newSiblingNodeChildren, treeContext);

            return Optional.of(new Pair<>(newKey, newSiblingNode.getPage().getPageNum()));
        }
    }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        if (splitInfo.isPresent()) {
            DataBox key = splitInfo.get().getFirst();
            long child = splitInfo.get().getSecond();
            Optional<Pair<DataBox, Long>> thisNodeSplitInfo = insert(key, child);

            if (!thisNodeSplitInfo.isPresent()) {
                return bulkLoad(data, fillFactor);
            } else {
                return thisNodeSplitInfo;
            }
