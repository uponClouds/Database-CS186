...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        if (keys.contains(key)) {
            String msg = String.format("The database is already has the key \"%s\".", key.toString());
        ...
        if (keys.size() <= metadata.getOrder() * 2) {
            ...
            // Hit the case 2 described in BPlusNode::put.

            List<DataBox> newSiblingKeys = keys.subList(metadata.getOrder(), keys.size());
            List<RecordId> newSiblingRids = rids.subList(metadata.getOrder(), rids.size());
            keys = keys.subList(0, metadata.getOrder());
            rids = rids.subList(0, metadata.getOrder());

            LeafNode newSibling = new LeafNode(metadata, bufferManager,
                    newSiblingKeys, newSiblingRids, rightSibling, treeContext);
            rightSibling = Optional.of(newSibling.getPage().getPageNum());
            sync();

            return Optional.of(new Pair<>(newSiblingKeys.get(0), newSibling.getPage().getPageNum()));
        }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
            float fillFactor) {
        // TODO(proj2): implement
        // Didn't make it at the first time.     ----By hjt
        int limit = (int) Math.ceil(metadata.getOrder() * fillFactor * 2);
        while (data.hasNext() && keys.size() < limit) {
            Pair<DataBox, RecordId> recordPair = data.next();
            keys.add(recordPair.getFirst());
            rids.add(recordPair.getSecond());
        }
        ...
        if (data.hasNext()) {
            ...
            Pair<DataBox, RecordId> recordIdPair = data.next();
            rightSiblingKeys.add(recordIdPair.getFirst());
            rightSiblingRids.add(recordIdPair.getSecond());
            LeafNode newRightSibling = new LeafNode(metadata, bufferManager,
                   rightSiblingKeys, rightSiblingRids, rightSibling, treeContext);
            rightSibling = Optional.of(newRightSibling.getPage().getPageNum());
            ret = Optional.of(new Pair<>(recordIdPair.getFirst(), rightSibling.get()));
        }
