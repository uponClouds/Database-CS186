...
public class SortOperator extends QueryOperator {
    ...
    public Run sortRun(Iterator<Record> records) {
        ...
        while (records.hasNext()) {
            runList.add(records.next());
        }
        runList.sort(new RecordComparator());

        return makeRun(runList);
    }
    ...
    public Run mergeSortedRuns(List<Run> runs) {
        ...
        for (int i = 0; i < iteratorList.size(); i++) {
            if (iteratorList.get(i).hasNext()) {
                priorityQueue.offer(new Pair<>(iteratorList.get(i).next(), i));
            }
        }

        Run outputRun = makeRun();
        while (!priorityQueue.isEmpty()) {
            Pair<Record, Integer> item = priorityQueue.poll();
            Record record = item.getFirst();
            int pos = item.getSecond();
            outputRun.add(record);

            if (iteratorList.get(pos).hasNext()) {
                priorityQueue.offer(new Pair<>(iteratorList.get(pos).next(), pos));
            }
        }
    ...
    public List<Run> mergePass(List<Run> runs) {
        ...
        int len = runs.size();
        int workBuffers = this.numBuffers - 1;

        for (int i = 0; i + workBuffers <= len; i += workBuffers) {
            mergeRuns.add(mergeSortedRuns(runs.subList(i, Math.min(len, i + workBuffers))));
        }
        return mergeRuns;
    }
    ...
    public Run sort() {
        ...
        while (sourceIterator.hasNext()) {
            runs.add(sortRun(getBlockIterator(sourceIterator, getSchema(), this.numBuffers)));
        }
        while (runs.size() > 1) {
            runs = mergePass(runs);
        }
