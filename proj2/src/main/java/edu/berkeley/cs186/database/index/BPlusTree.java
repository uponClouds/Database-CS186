...
public class BPlusTree {
     ...
     * instead of an Optional. That is, if you get(k) returns Optional.empty(),
     * then scanEqual(k) returns an empty iterator. If you get(k) returns
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        // Split the root and make a new root.
        splitInfo.ifPresent(dataBoxLongPair -> makeNewRoot(dataBoxLongPair.getFirst(), dataBoxLongPair.getSecond()));

    }

    /**
     * Make a new root, which is the case 2 described in BPlusNode::put
     *
     * @author hjt
     * @param key
     * @param child
     */
    private void makeNewRoot(DataBox key, Long child) {
        List<DataBox> keys = new ArrayList<>();
        ...
        children.add(child);
        BPlusNode newRootNode = new InnerNode(metadata, bufferManager, keys, children, lockContext);
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        if (scanAll().hasNext()) {
            String msg = "BulkLoad must be called in an empty tree.";
            throw new RuntimeException(msg);
        }
        while (data.hasNext()) {
            Optional<Pair<DataBox, Long>> splitInfo = root.bulkLoad(data, fillFactor);
            splitInfo.ifPresent(info -> makeNewRoot(info.getFirst(), info.getSecond()));
        }
        return;
    }
    ...
    // Iterator ////////////////////////////////////////////////////////////////
    // This task (Task3 in proj1) was hard for me to implement at the first time.     -----by hjt
